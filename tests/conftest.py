...
async def ensure_covenant_compliance(user_id: str):
    """
...
async def covenant_compliant_project(tmp_path):
    """
    Fixture that creates a user context and ensures covenant compliance.

    Returns the user_id that can be used with tools requiring
    ...
    await ensure_covenant_compliance(user_id)
