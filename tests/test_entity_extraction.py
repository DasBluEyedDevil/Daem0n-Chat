...
class TestExtractedEntityModel:
    ...
    def test_extracted_entity_has_required_fields(self):
        ...
        assert entity.user_id == "/test/project"
...
async def covenant_compliant_project_for_entities(tmp_path):
    ...
    # This creates the DB at the right path: user_id/.daem0nmcp/storage
...
async def test_mcp_recall_by_entity(covenant_compliant_project_for_entities):
    ...
    await server.remember(
        ...
        user_id=covenant_compliant_project_for_entities
    )
...
@pytest.mark.asyncio
async def test_mcp_recall_by_entity_missing_user_id():
    """Test that recall_by_entity requires user_id."""
    from daem0nmcp import server

    # Clear any default project path
    original_default = server._default_user_id
    server._default_user_id = None
    server._user_contexts.clear()
    ...
    try:
        ...
        server._default_user_id = original_default


@pytest.mark.asyncio
async def test_mcp_list_entities_missing_user_id():
    """Test that list_entities requires user_id."""
    from daem0nmcp import server
    ...
    try:
        result = await server.list_entities(user_id=None)
        assert "error" in result
        assert result["error"] == "MISSING_PROJECT_PATH"
    finally:
        server._default_user_id = original_default
...
@pytest.mark.asyncio
async def test_mcp_backfill_entities_missing_user_id():
    """Test that backfill_entities requires user_id."""
    from daem0nmcp import server
    ...
    try:
        result = await server.backfill_entities(user_id=None)
        assert "error" in result
        assert result["error"] == "MISSING_PROJECT_PATH"
    finally:
        server._default_user_id = original_default
