...
def _make_memory(
    ...
    # Support both old category and new categories
    mem.categories = categories or ["goal"]
    mem.category = mem.categories[0] if mem.categories else "goal"  # deprecated
    mem.context = {}
    mem.tags = []
    return mem
...
def _make_recall_result(memories=None):
    """Build a recall() return dict with flat memories list (new format).

    Each memory in *memories* is a dict with at least ``id`` and
    optionally ``worked``, ``content``, ``outcome``, ``categories``.
    """
    if memories is None:
        memories = []
    # Ensure each memory has categories
    for m in memories:
    ...
    return {
        ...
        "has_more": False,
        "summary": None,
        "memories": memories,  # New flat format
    }
