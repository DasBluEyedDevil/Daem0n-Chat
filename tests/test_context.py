...
class TestUserContextConcurrency:
    ...
    async def test_concurrent_context_creation_uses_lock(self, temp_projects):
        """Verify that concurrent calls to get_user_context don't race."""
        from daem0nmcp.server import get_user_context, _user_contexts, _context_locks

        # Clear existing contexts and locks
        _user_contexts.clear()
        _context_locks.clear()

        user_id = temp_projects[0]
        ...
        with patch.object(DatabaseManager, 'init_db', counting_init):
            ...
            tasks = [get_user_context(user_id) for _ in range(5)]
...
class TestUserContextEviction:
    ...
    async def test_lru_eviction_when_max_contexts_exceeded(self, temp_projects):
        ...
        from daem0nmcp.server import (
            get_user_context, _user_contexts,
        ...
        for i, user_id in enumerate(temp_projects[:MAX_PROJECT_CONTEXTS + 2]):
        ...
        assert len(_user_contexts) <= MAX_PROJECT_CONTEXTS
        assert evicted >= 2
...
class TestPathResolution:
    ...
    async def test_different_projects_get_different_contexts(self):
        ...
        import tempfile
        from daem0nmcp.server import get_user_context, _user_contexts

        _user_contexts.clear()

        with tempfile.TemporaryDirectory() as dir1:
            with tempfile.TemporaryDirectory() as dir2:
                ctx1 = await get_user_context(dir1)
                ctx2 = await get_user_context(dir2)

                assert ctx1 is not ctx2
                assert ctx1.user_id != ctx2.user_id
                assert len(_user_contexts) == 2
