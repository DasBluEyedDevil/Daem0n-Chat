...
class TestFullCovenantFlow:
    ...
    async def test_complete_covenant_flow(self, db_manager):
        ...
        user_id = str(db_manager.storage_path.parent.parent)

        # 1. COMMUNION - get_briefing
        briefing = await server.get_briefing(user_id=user_id)
        assert briefing["status"] == "ready"

        # 2. Verify recall works after briefing
        recall_result = await server.recall(topic="test", user_id=user_id)
    ...
    async def test_enforcement_blocks_are_recoverable(self, db_manager):
        ...
        result = await server.recall(topic="test", user_id=user_id)
    ...
    async def test_parallel_preflight_tools(self, db_manager):
        ...
        results = await asyncio.gather(
            server.context_check(description="editing test.py", user_id=user_id),
            server.recall_for_file(file_path="test.py", user_id=user_id),
