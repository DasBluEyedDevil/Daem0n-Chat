...
async def covenant_compliant_project(temp_storage):
    ...
    await server.get_briefing(user_id=temp_storage)
    yield temp_storage
...
class TestEntityExtractionDuringRemember:
    ...
    @pytest.mark.asyncio
    async def test_remember_extracts_entities_when_user_id_provided(
        self, memory_manager, entity_manager, temp_storage
    ):
        """Remember should extract entities when user_id is provided."""
