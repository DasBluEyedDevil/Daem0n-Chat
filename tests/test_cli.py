...
def run_cli(*args, env=None, user_id=None):
    ...
    if user_id:
        cmd.extend(["--project-path", user_id])
    cmd.extend(args)
...
class TestBriefingCommand:
    ...
    def test_briefing_json_output(self, temp_project):
        ...
        result = run_cli("--json", "briefing", user_id=temp_project)
        assert result.returncode == 0
    ...
    def test_briefing_text_output(self, temp_project):
        ...
        result = run_cli("briefing", user_id=temp_project)
        assert result.returncode == 0
...
class TestStatusCommand:
    ...
    def test_status_json_output(self, temp_project):
        ...
        result = run_cli("--json", "status", user_id=temp_project)
        assert result.returncode == 0
    ...
    def test_status_text_output(self, temp_project):
        ...
        result = run_cli("status", user_id=temp_project)
        assert result.returncode == 0
...
class TestMigrateCommand:
    ...
    def test_migrate_creates_database(self, temp_project):
        ...
        result = run_cli("--json", "migrate", user_id=temp_project)
        assert result.returncode == 0
    ...
    def test_migrate_idempotent(self, temp_project):
        ...
        # First run
        result1 = run_cli("--json", "migrate", user_id=temp_project)
        assert result1.returncode == 0

        # Second run should also succeed
        result2 = run_cli("--json", "migrate", user_id=temp_project)
...
class TestScanTodosCommand:
    ...
    def test_scan_todos_empty_directory(self, temp_project):
        ...
        result = run_cli("--json", "scan-todos", "--path", temp_project, user_id=temp_project)
        assert result.returncode == 0
...
class TestCheckCommand:
    ...
    def test_check_nonexistent_file(self, temp_project):
        ...
        result = run_cli("--json", "check", "nonexistent.py", user_id=temp_project)
        assert result.returncode == 0
    ...
    def test_check_existing_file(self, temp_project):
        ...
        result = run_cli("--json", "check", str(test_file), user_id=temp_project)
        assert result.returncode == 0
...
class TestHooksCommands:
    ...
    def test_install_hooks_no_git_repo(self, temp_project):
        ...
        result = run_cli("install-hooks", user_id=temp_project)
    ...
    def test_uninstall_hooks_no_git_repo(self, temp_project):
        ...
        result = run_cli("uninstall-hooks", user_id=temp_project)
...
class TestProjectPathOption:
    ...
    def test_user_id_overrides_default(self, temp_project):
    ...
    def test_different_projects_isolated(self):
        ...
        try:
            ...
            result1 = run_cli("--json", "briefing", user_id=temp1)
            result2 = run_cli("--json", "briefing", user_id=temp2)
...
class TestJSONOutputOption:
    ...
    def test_json_output_is_valid_json(self, temp_project):
        ...
        for cmd in commands:
            result = run_cli("--json", *cmd, user_id=temp_project)
...
class TestErrorHandling:
    ...
    def test_missing_required_args(self, temp_project):
        ...
        result = run_cli("check", user_id=temp_project)
        assert result.returncode != 0

        # record-outcome requires memory_id and outcome
        result = run_cli("record-outcome", user_id=temp_project)
        assert result.returncode != 0
