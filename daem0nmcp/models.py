...
Tables:
- memories: Stores conversational memories with multi-category support
...
# =============================================================================
# Conversational memory categories -- single source of truth
# =============================================================================

VALID_CATEGORIES = frozenset({
    'fact',          # concrete personal info (name, location, job)
    'preference',    # likes, dislikes, opinions
    'interest',      # hobbies, curiosities, passions
    'goal',          # aspirations, plans, things working toward
    'concern',       # worries, stresses, anxieties
    'event',         # life events and milestones
    'relationship',  # people, pets, connections
    'emotion',       # how user felt about things
    'routine',       # recurring patterns and habits
    'context',       # situational/temporary info (moving, job searching)
})

# Permanence mapping -- which categories resist memory decay
...
class Memory(Base):
    ...
    Conversational Categories (multi-category support via JSON array):
    - fact: Concrete personal info (name, location, job)
    - preference: Likes, dislikes, opinions
    - interest: Hobbies, curiosities, passions
    - goal: Aspirations, plans, things working toward
    ...
    Permanence is determined by PERMANENT_CATEGORIES:
    - Permanent (no decay): fact, preference, relationship, routine, event
    - Slow decay (90-day half-life): interest, goal
    - Fast decay (30-day half-life): emotion, concern, context
    """
    __tablename__ = "memories"

    id = Column(Integer, primary_key=True, index=True)

    # DEPRECATED: Single category string (kept for migration compatibility)
    # Use 'categories' JSON column for new code
    category = Column(String, nullable=True, index=True)  # deprecated: use categories
    ...
    # Permanent flag - memories with PERMANENT_CATEGORIES don't decay
    # Auto-set based on categories, but can be overridden
