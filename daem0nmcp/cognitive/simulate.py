...
# Conversational memory categories (new format uses flat 'memories' list)
# These are kept for backward compatibility with legacy recall format
_RECALL_CATEGORIES = {
    'fact': 'Recall facts about the user',
    'preference': 'Recall user preferences',
    'interest': 'Recall user interests',
    'goal': 'Recall user goals and aspirations',
    'concern': 'Recall user concerns and worries',
    'event': 'Recall life events',
    'relationship': 'Recall user relationships',
    'emotion': 'Recall emotional context',
    'routine': 'Recall user routines',
    'context': 'Recall current situational context',
}

# Legacy category keys for backward compatibility with old recall format
_LEGACY_RECALL_CATEGORIES = ("decisions", "patterns", "warnings", "learnings")


def _extract_memories_from_recall(recall_result: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Flatten the recall result into a single memory list.

    Handles both the new flat 'memories' list format and the legacy
    category-keyed dict format for backward compatibility.
    """
    memories: List[Dict[str, Any]] = []

    # New format: flat list under 'memories' key
    if "memories" in recall_result and isinstance(recall_result["memories"], list):
        return recall_result["memories"]

    # Legacy format: categorized_memories wrapper
    ...
    # Legacy format: category keys at the top level (decisions, patterns, etc.)
    for cat_key in _LEGACY_RECALL_CATEGORIES:
        cat_list = recall_result.get(cat_key)
        if isinstance(cat_list, list):
            for mem in cat_list:
                # Tag the categories onto each memory dict for downstream use
                if "categories" not in mem:
                    # Map old category to new format
                    old_cat = cat_key.rstrip("s")  # decisions -> decision
                    mem["categories"] = [old_cat]
                memories.append(mem)
