...
try:
    ...
    from ..context_manager import (
        UserContext, get_user_context, _default_user_id,
        _missing_user_id_error, _user_contexts,
    )
...
def _extract_project_identity(user_id: str) -> Optional[str]:
    """
...
def _extract_architecture(user_id: str) -> Optional[str]:
    """
...
def _extract_conventions(user_id: str) -> Optional[str]:
    """
...
def _extract_entry_points(user_id: str) -> Optional[str]:
    """
...
def _scan_todos_for_bootstrap(user_id: str, limit: int = 20) -> Optional[str]:
    """
    ...
    todos = _scan_for_todos(user_id, max_files=200)
...
def _extract_project_instructions(user_id: str) -> Optional[str]:
    """
...
# ============================================================================
def _get_git_history_summary(user_id: str, limit: int = 30) -> Optional[str]:
    """Get a summary of git history for bootstrapping context.

    Args:
        user_id: Directory to run git commands in
...
def _get_git_changes(since_date: Optional[datetime] = None, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Get git changes since a given date.

    Args:
        since_date: Only show commits since this date
        user_id: Directory to run git commands in (defaults to CWD)
    """
    try:
        # Use user_id as working directory for git commands
        cwd = user_id if user_id else None
...
# ============================================================================
async def _bootstrap_project_context(ctx: UserContext) -> Dict[str, Any]:
    """
    ...
    extractors = [
        (
            "project_identity",
            lambda: _extract_project_identity(ctx.user_id),
            "pattern",
        ...
        (
            "architecture",
            lambda: _extract_architecture(ctx.user_id),
            "pattern",
        ...
        (
            "conventions",
            lambda: _extract_conventions(ctx.user_id),
            "pattern",
        ...
        (
            ...
            lambda: _extract_project_instructions(ctx.user_id),
            "pattern",
        ...
        (
            "git_evolution",
            lambda: _get_git_history_summary(ctx.user_id, limit=30),
            "learning",
        ...
        (
            "known_issues",
            lambda: _scan_todos_for_bootstrap(ctx.user_id, limit=20),
            "warning",
        ...
        (
            "entry_points",
            lambda: _extract_entry_points(ctx.user_id),
            "learning",
    ...
    for name, extractor, category, rationale, tags in extractors:
        try:
            ...
            if content:
                ...
                logger.info(f"Bootstrapped {name} for {ctx.user_id}")
            else:
...
async def _fetch_recent_context(ctx: UserContext) -> Dict[str, Any]:
    """
...
async def _fetch_dream_sessions(ctx: UserContext, limit: int = 5) -> List[Dict[str, Any]]:
    """
...
async def _prefetch_focus_areas(
    ...
    for area in focus_areas[:4]:  # Limit to 4 areas
        memories = await ctx.memory_manager.recall(
            area, limit=3, user_id=ctx.user_id,
...
async def _get_linked_projects_summary(ctx: UserContext) -> List[Dict[str, Any]]:
    """
    Get summary of linked projects with warning/memory counts.

    Args:
        ctx: Project context with db_manager and user_id
    ...
    links = await link_mgr.list_linked_projects(ctx.user_id)
...
async def get_briefing(
    ...
    try:
        ...
        full_context = await acm.get_active_context(ctx.user_id, condensed=True)
        ...
        await acm.cleanup_expired(ctx.user_id)
    except Exception as e:
...
async def get_covenant_status_visual(
    ...
    result = await get_covenant_status(user_id=user_id)
...
async def context_check(
    ...
    memories = await ctx.memory_manager.recall(description, limit=5, user_id=ctx.user_id)
    if memories is None:
        memories = {}
    ...
    token = PreflightToken.issue(
        action=description,
        session_id=get_session_id(ctx.user_id),
        user_id=ctx.user_id,
    )
...
async def check_for_updates(
    ...
    Args:
        ...
        user_id: Project context path (uses current directory if not specified)
...
async def health(
    ...
    result = {
        ...
        "contexts_cached": len(_user_contexts),
