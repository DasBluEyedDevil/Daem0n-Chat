"""
UserContext lifecycle management for Daem0nMCP.

This module manages the creation, caching, eviction, and cleanup of
per-user context objects. Each UserContext holds the DatabaseManager,
MemoryManager, and RulesEngine for a single user.

Import hierarchy position:
    mcp_instance  (shared FastMCP -- no business imports)
        <- context_manager  (THIS MODULE -- UserContext lifecycle)
...
# ============================================================================
# USER CONTEXT MANAGEMENT - Support multiple users via HTTP transport
# ============================================================================
@dataclass
class UserContext:
    """Holds all managers for a specific user."""
    user_id: str
    storage_path: str
...
# Default user ID (ONLY used if DAEM0NMCP_USER_ID is explicitly set)
_default_user_id: Optional[str] = os.environ.get('DAEM0NMCP_USER_ID')

# Configuration constants (read from settings)
MAX_USER_CONTEXTS = settings.max_project_contexts
CONTEXT_TTL_SECONDS = settings.context_ttl_seconds


def _get_context_for_covenant(user_id: str) -> Optional[UserContext]:
    """
    Get a user context for covenant enforcement.

    This is called by the covenant decorators to check session state.
    """
    try:
        normalized = str(Path(user_id).resolve())
        return _user_contexts.get(normalized)
    except Exception:
        return None
...
def _get_context_state_for_middleware(user_id: Optional[str]) -> Optional[Dict[str, Any]]:
    """
    ...
    Args:
        user_id: User ID to look up (may be None for some tools)
    ...
    ctx = _get_context_for_covenant(user_id)
    if ctx is None:
        return None
...
def _missing_user_id_error() -> Dict[str, Any]:
    """Return an error dict when user_id is not provided."""
    return {
        "error": "MISSING_USER_ID",
        "message": (
            "The user_id parameter is REQUIRED. "
            "The Daem0n serves multiple users - you must specify which user's memories to access. "
            "Pass a unique identifier for the user. "
            "Example: user_id='alice' or user_id='/home/alice/projects/myapp'"
        ),
        "hint": "Provide a unique user identifier to access their memories."
    }


def _check_covenant_communion(user_id: str) -> Optional[Dict[str, Any]]:
    """
    Check if communion (get_briefing) was performed for this user.
...
def _check_covenant_counsel(tool_name: str, user_id: str) -> Optional[Dict[str, Any]]:
    """
    Check if counsel (context_check) was sought for this user.
    ...
    communion_violation = _check_covenant_communion(user_id)
...
def _get_storage_for_user(user_id: str) -> str:
    """Get the storage path for a user."""
    return str(Path(user_id) / ".daem0nmcp" / "storage")


def _resolve_within_user(user_root: str, target_path: Optional[str]) -> Tuple[Optional[Path], Optional[str]]:
    """
    Resolve a path and ensure it stays within the user root.

    Args:
        user_root: The user root directory
        target_path: Optional path relative to user root
    ...
    try:
        ...
        logger.warning(f"Path resolution failed for '{user_root}' / '{target_path}': {e}")
        return None, f"Invalid path: {e}"

    try:
        resolved.relative_to(root)
    except ValueError:
        return None, "Path must be within the user root"
...
async def _track_task_context(ctx: UserContext) -> None:
    ...
    async with _task_contexts_lock:
        ...
        counts[ctx.user_id] = counts.get(ctx.user_id, 0) + 1
...
async def get_user_context(user_id: Optional[str] = None) -> UserContext:
    """
    Get or create a UserContext for the given user ID.
    ...
    This enables the HTTP server to handle multiple users simultaneously,
    each with their own isolated database.

    Args:
        user_id: Unique identifier for the user. If None, uses default.

    Returns:
        UserContext with initialized managers for that user.
    """
    ...
    # Normalize for consistent caching - validate user_id is not None
    if not user_id:
        raise ValueError("user_id is required when DAEM0NMCP_USER_ID is not set")
    normalized = _normalize_path(user_id)
    ...
    async with _contexts_lock.read():
        if normalized in _user_contexts:
            ctx = _user_contexts[normalized]
            if ctx.initialized:
                now = time.time()
                ctx.last_accessed = now
                # Opportunistic eviction: trigger background cleanup if over limit
                if len(_user_contexts) > MAX_USER_CONTEXTS:
                    asyncio.create_task(evict_stale_contexts())
                else:
    ...
    async with _contexts_lock.write():
        ...
        # Get or create per-user lock for initialization
        if normalized not in _context_locks:
            _context_locks[normalized] = asyncio.Lock()
        lock = _context_locks[normalized]

    # Initialize under user-specific lock (outside RWLock to avoid holding it during I/O)
    async with lock:
        # Triple-check: another task may have initialized while we waited for per-user lock
        if normalized in _user_contexts:
            ctx = _user_contexts[normalized]
            if ctx.initialized:
                now = time.time()
        ...
        # Create new context
        storage_path = _get_storage_for_user(normalized)
        ...
        async with _contexts_lock.write():
            _user_contexts[normalized] = ctx
        logger.info(f"Created user context for: {normalized} (storage: {storage_path})")
...
async def evict_stale_contexts() -> int:
    """
    Evict stale user contexts based on LRU and TTL policies.
    ...
    ttl_candidates = []
    async with _contexts_lock.read():
        for path, ctx in _user_contexts.items():
    ...
    while True:
        async with _contexts_lock.write():
            ...
            ctx = _user_contexts.get(oldest_path)
            ...
            async with ctx.lock:
                ...
                _user_contexts.pop(oldest_path, None)
    ...
    async with _contexts_lock.write():
        orphaned_locks = set(_context_locks.keys()) - set(_user_contexts.keys())
...
async def cleanup_all_contexts():
    """Clean up all user contexts on shutdown."""
    for path, ctx in _user_contexts.items():
        try:
            await ctx.db_manager.close()
...
async def hold_context(ctx: "UserContext"):
    """
    Hold a user context's active_requests for the duration of a block.
