    ...
    **kwargs,
    ...
    if action == "add_rule":
        ...
        return await _do_add_rule(
            user_id, trigger, must_do, must_not,
        ...
        return await _do_update_rule(
            user_id, rule_id, must_do, must_not,
            ask_first, warnings, priority, enabled,
        )

    elif action == "list_rules":
        return await _do_list_rules(user_id, enabled_only, limit)
...
async def _do_list_rules(
    user_id: str, enabled_only: bool, limit: int
) -> Any:
    """List all configured rules."""
    from ..server import list_rules

    return await list_rules(
        enabled_only=enabled_only, limit=limit, user_id=user_id
    )
