...
async def dispatch(
    ...
    **kwargs,
    ...
    if action == "index":
        return await _do_index(user_id, path, patterns)

    elif action == "find":
        if not query:
            raise MissingParamError("query", action)
        return await _do_find(user_id, query, limit)

    elif action == "impact":
        if not entity_name:
            raise MissingParamError("entity_name", action)
        return await _do_impact(user_id, entity_name)

    elif action == "todos":
        return await _do_todos(user_id, path, auto_remember, types)

    elif action == "refactor":
        if not file_path:
            raise MissingParamError("file_path", action)
        return await _do_refactor(user_id, file_path)
...
async def _do_index(
    ...
    return await index_project(
        path=path, patterns=patterns, user_id=user_id
    )
