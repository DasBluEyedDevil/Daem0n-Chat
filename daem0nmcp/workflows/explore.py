    ...
    **kwargs,
    ...
    if action == "related":
        ...
        return await _do_related(
            user_id, memory_id, relationship_types, direction, max_depth
        )
        ...
        return await _do_chain(
            user_id, start_memory_id, end_memory_id, max_depth
        )

    elif action == "graph":
        return await _do_graph(
            user_id, memory_ids, topic, format, visual, include_orphans
        )

    elif action == "stats":
        return await _do_stats(user_id)

    elif action == "communities":
        return await _do_communities(
            user_id, level, parent_community_id, visual
        )

    elif action == "community_detail":
        if community_id is None:
            raise MissingParamError("community_id", action)
        return await _do_community_detail(user_id, community_id)

    elif action == "rebuild_communities":
        return await _do_rebuild_communities(
            user_id, min_community_size, resolution
        )

    elif action == "entities":
        return await _do_entities(user_id, entity_type, limit)

    elif action == "backfill_entities":
        return await _do_backfill_entities(user_id)
        ...
        if memory_id is None:
            raise MissingParamError("memory_id", action)
        return await _do_versions(user_id, memory_id, limit)
        ...
        return await _do_at_time(user_id, memory_id, timestamp)
...
async def _do_stats(user_id: str) -> Dict[str, Any]:
    """Get knowledge graph metrics."""
    from ..server import get_graph_stats

    return await get_graph_stats(user_id=user_id)
...
async def _do_rebuild_communities(
    user_id: str, min_community_size: int, resolution: float
) -> Dict[str, Any]:
...
async def _do_entities(
    user_id: str, entity_type: Optional[str], limit: int
) -> Dict[str, Any]:
    """List most frequently mentioned entities."""
    from ..server import list_entities

    return await list_entities(
        entity_type=entity_type, limit=limit, user_id=user_id
    )


async def _do_backfill_entities(user_id: str) -> Dict[str, Any]:
    """Extract entities from all existing memories."""
    from ..server import backfill_entities

    return await backfill_entities(user_id=user_id)
