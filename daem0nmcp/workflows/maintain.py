    ...
    **kwargs,
    ...
    if action == "prune":
        return await _do_prune(
            user_id, older_than_days, categories,
        ...
        return await _do_archive(user_id, memory_id, archived)

    elif action == "cleanup":
        return await _do_cleanup(user_id, dry_run, merge_duplicates)

    elif action == "compact":
        if not summary:
            raise MissingParamError("summary", action)
        return await _do_compact(
            user_id, summary, limit, topic, dry_run
        )

    elif action == "rebuild_index":
        return await _do_rebuild_index(user_id)

    elif action == "export":
        return await _do_export(user_id, include_vectors)

    elif action == "import_data":
        if data is None:
            raise MissingParamError("data", action)
        return await _do_import(user_id, data, merge)

    elif action == "link_project":
        if not linked_path:
            raise MissingParamError("linked_path", action)
        return await _do_link_project(
            user_id, linked_path, relationship, label
        )

    elif action == "unlink_project":
        if not linked_path:
            raise MissingParamError("linked_path", action)
        return await _do_unlink_project(user_id, linked_path)

    elif action == "list_projects":
        return await _do_list_projects(user_id)

    elif action == "consolidate":
        return await _do_consolidate(user_id, archive_sources)

    elif action == "purge_dream_spam":
        return await _do_purge_dream_spam(user_id, dry_run)
...
async def _do_archive(
    user_id: str, memory_id: int, archived: bool
) -> Dict[str, Any]:
    """Archive/unarchive a memory."""
    from ..server import archive_memory

    return await archive_memory(
        memory_id=memory_id, archived=archived, user_id=user_id
    )


async def _do_cleanup(
    user_id: str, dry_run: bool, merge_duplicates: bool
) -> Dict[str, Any]:
...
async def _do_unlink_project(
    user_id: str, linked_path: str
) -> Dict[str, Any]:
    """Remove project link."""
    from ..server import unlink_projects

    return await unlink_projects(
        linked_path=linked_path, user_id=user_id
    )


async def _do_list_projects(user_id: str) -> Dict[str, Any]:
    """List all linked projects."""
    from ..server import list_linked_projects

    return await list_linked_projects(user_id=user_id)


async def _do_consolidate(
    user_id: str, archive_sources: bool
) -> Dict[str, Any]:
    ...
    return await consolidate_linked_databases(
        archive_sources=archive_sources, user_id=user_id
    )
...
async def _do_purge_dream_spam(
    ...
    try:
        ...
        from daem0nmcp.context_manager import get_user_context
