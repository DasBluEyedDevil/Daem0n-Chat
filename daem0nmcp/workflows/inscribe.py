    ...
    *,
    ...
    if action == "remember":
        ...
        return await _do_remember(
            user_id, category, content, rationale,
        ...
        return await _do_remember_batch(user_id, memories)
        ...
        return await _do_link(
            user_id, source_id, target_id, relationship, description
        )
        ...
        return await _do_pin(user_id, memory_id, pinned)

    elif action == "activate":
        if memory_id is None:
            raise MissingParamError("memory_id", action)
        return await _do_activate(
            user_id, memory_id, reason, priority, expires_in_hours
        )

    elif action == "deactivate":
        if memory_id is None:
            raise MissingParamError("memory_id", action)
        return await _do_deactivate(user_id, memory_id)

    elif action == "clear_active":
        return await _do_clear_active(user_id)
        ...
        return await _do_ingest(user_id, url, topic, chunk_size)
...
async def _do_remember_batch(
    ...
    return await remember_batch(
        memories=memories, user_id=user_id
    )
...
async def _do_pin(
    user_id: str, memory_id: int, pinned: bool
) -> Dict[str, Any]:
    """Pin/unpin a memory."""
    from ..server import pin_memory

    return await pin_memory(
        memory_id=memory_id, pinned=pinned, user_id=user_id
    )
...
async def _do_clear_active(user_id: str) -> Dict[str, Any]:
    """Clear all active context memories."""
    from ..server import clear_active_context

    return await clear_active_context(user_id=user_id)


async def _do_ingest(
    user_id: str, url: str, topic: str, chunk_size: int
) -> Dict[str, Any]:
