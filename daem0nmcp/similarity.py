...
def detect_conflict(
    ...
    for i, mem in enumerate(existing_memories):
        ...
        if similarity >= similarity_threshold:
            ...
            if not categories and mem.get('category'):
                categories = [mem.get('category')]
            ...
            elif 'concern' in categories or 'warning' in categories:  # support old 'warning' category
                conflict_info['conflict_type'] = 'existing_concern'
                conflict_info['warning'] = f"Existing concern about this: {mem.get('content')}"
            elif similarity > 0.8:
