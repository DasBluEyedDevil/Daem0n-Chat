...
async def check_file(filepath: str, db: DatabaseManager, memory: MemoryManager, rules: RulesEngine) -> dict:
    ...
    file_memories = await memory.recall_for_file(filepath, user_id=settings.project_root)
...
async def get_enforcement_status(db: DatabaseManager, memory: MemoryManager, user_id: str) -> dict:
...
async def run_precommit(checker, staged_files: list, user_id: str, interactive: bool, json_output: bool) -> int:
    """
    ...
    try:
        ...
        result = await checker.check(staged_files=staged_files, user_id=user_id)
    except Exception as e:
        if json_output:
...
def main():
    ...
    if args.user_id:
        os.environ['DAEM0NMCP_PROJECT_ROOT'] = args.user_id
    ...
    if args.command == "install-opencode":
        ...
        user_id = args.user_id or os.getcwd()
        ...
        user_id = args.user_id or os.getcwd()
        success, message = install_hooks(user_id, force=args.force)
        if args.json:
            print(json.dumps({"success": success, "message": message}))
        else:
            print(message)
        ...
        user_id = args.user_id or os.getcwd()
        success, message = uninstall_hooks(user_id)
        if args.json:
            print(json.dumps({"success": success, "message": message}))
        else:
            print(message)
    ...
    if args.command == "check":
        ...
        user_id = args.user_id or os.getcwd()
        try:
            result = asyncio.run(get_enforcement_status(db, memory, user_id))
        except Exception as e:
            if args.json:
        ...
        exit_code = asyncio.run(run_precommit(checker, staged_files, user_id, args.interactive, args.json))
        sys.exit(exit_code)
        ...
        user_id = Path(args.user_id or os.getcwd()).resolve()
        ...
        async def run_watcher():
            ...
            print(f"Starting file watcher for: {user_id}")
        ...
        user_id = Path(args.user_id or os.getcwd()).resolve()
        target_path = args.path or str(user_id)
        ...
        user_id = args.user_id or os.getcwd()
        storage_path_resolved = Path(user_id).resolve() / ".daem0nmcp" / "storage"
