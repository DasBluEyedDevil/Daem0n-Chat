...
class CovenantViolation:
    ...
    def communion_required(user_id: str) -> Dict[str, Any]:
        ...
        Args:
            user_id: The project path being accessed
...
class CovenantTransform:
    ...
    def check_tool_access(
        ...
        Args:
            ...
            user_id: Project root path (may be None for some tools)
        ...
        if tool_name in self.communion_required_tools:
            if state is None or not state.get("briefed", False):
                logger.info(f"Communion required for tool '{tool_name}' on project '{user_id}'")
                return CovenantViolation.communion_required(user_id or "unknown")
        ...
        if tool_name in self.counsel_required_tools:
            ...
            if not context_checks:
                ...
                return CovenantViolation.counsel_required(tool_name, user_id or "unknown")
...
class CovenantMiddleware(Middleware if _FASTMCP_MIDDLEWARE_AVAILABLE else object):
    ...
    The middleware will:
    1. Extract tool_name and user_id from the request
    ...
    async def on_call_tool(
        ...
        logger.debug(f"CovenantMiddleware: Checking tool '{tool_name}' for project '{user_id}'")
