...
from .models import Memory, MemoryRelationship, MemoryVersion, VALID_CATEGORIES, PERMANENT_CATEGORIES, SLOW_DECAY_CATEGORIES, FAST_DECAY_CATEGORIES
...
def _infer_tags(content: str, categories: List[str], existing_tags: Optional[List[str]] = None) -> List[str]:
    """
    Infer semantic tags from memory content and categories.

    Auto-detects common conversational patterns to improve search recall:
    - emotional: sentiment keywords (happy, sad, stressed, excited, worried, anxious)
    - temporal: date/time references (yesterday, next week, last month, date patterns)
    - social: mentions of people/relationships (my sister, friend, partner, colleague)
    - aspirational: future + desire words (want to, hope to, plan to, going to)
    ...
    # Emotional patterns - sentiment keywords
    emotional_pattern = r'\b(happy|sad|stressed|excited|worried|anxious|angry|frustrated|grateful|proud|nervous|overwhelmed|relieved|disappointed)\b'
    if re.search(emotional_pattern, content_lower):
        if 'emotional' not in existing:
            inferred.append('emotional')

    # Temporal patterns - date/time references
    temporal_pattern = r'\b(yesterday|tomorrow|today|next\s+week|last\s+week|next\s+month|last\s+month|next\s+year|last\s+year|this\s+morning|tonight|monday|tuesday|wednesday|thursday|friday|saturday|sunday|january|february|march|april|may|june|july|august|september|october|november|december|\d{1,2}/\d{1,2}|\d{4})\b'
    if re.search(temporal_pattern, content_lower):
        if 'temporal' not in existing:
            inferred.append('temporal')

    # Social patterns - mentions of people/relationships
    social_pattern = r'\b(my\s+(sister|brother|mom|mother|dad|father|wife|husband|partner|friend|colleague|boss|coworker|roommate|neighbor|daughter|son|child|kid|family|parents|grandma|grandpa|aunt|uncle|cousin))\b'
    if re.search(social_pattern, content_lower):
        if 'social' not in existing:
            inferred.append('social')

    # Aspirational patterns - future + desire words
    aspirational_pattern = r'\b(want\s+to|hope\s+to|plan\s+to|going\s+to|would\s+like\s+to|dream\s+of|aspire\s+to|wish\s+to|aim\s+to|intend\s+to)\b'
    if re.search(aspirational_pattern, content_lower):
        if 'aspirational' not in existing:
            inferred.append('aspirational')

    # Concern category auto-tag (maps from old 'warning' behavior)
    if 'concern' in categories:
        if 'concern' not in existing:
            inferred.append('concern')

    # Explicit concern mentions in non-concern categories
    concern_pattern = r'\b(warn|avoid|worry|concern|anxious|stress)\b|don\'t'
    if 'concern' not in categories and re.search(concern_pattern, content_lower):
        if 'concern' not in existing:
            inferred.append('concern')
...
class MemoryManager:
    ...
    async def remember(
        ...
        Args:
            categories: One or more categories from VALID_CATEGORIES:
                       fact, preference, interest, goal, concern, event,
                       relationship, emotion, routine, context.
        ...
        if invalid:
            return {"error": f"Invalid categories: {invalid}. Must be from: {sorted(VALID_CATEGORIES)}"}

        # Infer semantic tags from content
        inferred_tags = _infer_tags(content, categories, tags)
        if inferred_tags:
        ...
        # Determine permanence based on PERMANENT_CATEGORIES
        # If any category in the memory is permanent, the memory is permanent
        is_permanent = bool(set(categories) & PERMANENT_CATEGORIES)
        ...
        async with self.db.get_session() as session:
            ...
            logger.info(f"Stored {categories}: {content[:50]}..." + (" [+qdrant]" if vector_embedding and self._qdrant else ""))
    ...
    async def remember_batch(
        ...
        Args:
            memories: List of memory dicts, each with:
                - categories: One or more categories from VALID_CATEGORIES (str or list)
                - content: The actual content to remember
        ...
        for i, mem in enumerate(memories):
            ...
            if not categories or invalid:
                results["errors"].append({
                    "index": i,
                    "error": f"Invalid or missing categories. Must be from: {sorted(VALID_CATEGORIES)}"
                })
            ...
            # Store normalized categories back
            mem = dict(mem)
        ...
        if project_path:
            try:
                ...
                goal_ids = [
                    ...
                    if j < len(created_ids) and 'goal' in mem.get("categories", [])
                ]
    ...
    async def recall(
        ...
        if found and cached_result is not None:
            ...
            recalled_ids = [m['id'] for m in cached_result.get('memories', [])]
        ...
        for mem_id, base_score in (search_results or []):
            ...
            # Get memory categories (from new JSON field or deprecated single field)
            mem_categories = getattr(mem, 'categories', None) or []
            if not mem_categories and hasattr(mem, 'category') and mem.category:
                mem_categories = [mem.category]

            # Apply category filter
            if categories:
                cats = set(categories)
                if include_warnings and 'concern' not in cats:
                    cats.add('concern')
                # Check if any of the memory's categories match the filter
                if not (set(mem_categories) & cats):
                    continue
            ...
            # Boost failed outcomes - they're valuable warnings
            if mem.worked is False:
                final_score *= FAILED_DECISION_BOOST

            # Boost concerns (was 'warning')
            if 'concern' in mem_categories:
        ...
        paginated_memories = scored_memories[offset:offset + limit]

        # Build flat list of memories (new format)
        results_list = []
        for mem, final_score, base_score, decay in paginated_memories:
            ...
            if mem.worked is False:
                mem_dict['_warning'] = f"This approach FAILED: {mem.outcome or 'no details recorded'}"

            results_list.append(mem_dict)

        total = len(results_list)

        # Generate summary
        summary_parts = []
        concern_count = sum(1 for m in results_list if 'concern' in m.get('categories', []))
        if concern_count:
            summary_parts.append(f"{concern_count} concerns")
        failed_count = sum(1 for m in results_list if m.get('worked') is False)
        if failed_count:
            summary_parts.append(f"{failed_count} failed approaches to avoid")
        routine_count = sum(1 for m in results_list if 'routine' in m.get('categories', []))
        if routine_count:
            summary_parts.append(f"{routine_count} routines to follow")

        # Increment recall_count for accessed memories (saliency tracking)
        recalled_ids = [m['id'] for m in results_list]
        await self._increment_recall_counts(recalled_ids)
            ...
            try:
                ...
                for linked_path, linked_db in linked_managers:
                    try:
                        ...
                        # Merge results, tagging with source (new flat list format)
                        for memory in linked_result.get("memories", []):
        ...
        # JIT compression metadata: check if assembled result text exceeds
        # token thresholds and add informational metadata to the result dict.
        # Individual memory objects remain uncompressed (structured data).
        # The metadata tells callers "this result set has X tokens, would
        # compress to Y".  JIT is an enhancement, never a gate.
        if settings.auto_zoom_enabled or settings.auto_zoom_shadow:
            try:
                all_content = "\n\n".join(
                    m.get("content", "")
                ...
                if all_content:
                    ...
                    if jit_result.get("threshold_triggered") is not None:
                        ...
                        logger.debug(
                            "[AUTO-ZOOM] JIT metadata: %s tier, %d -> %d tokens",
                            jit_result["threshold_triggered"],
                            jit_result["original_tokens"],
                            jit_result["compressed_tokens"],
                        )
            except Exception:
                logger.warning(
                    "[AUTO-ZOOM] JIT compression metadata failed, continuing without",
                    exc_info=True,
                )
    ...
    async def record_outcome(
        ...
        # If it failed, suggest creating an explicit concern
        if not worked:
            response["suggestion"] = {
                "action": "consider_concern",
                "message": "This failure will boost this memory in future recalls. Consider also creating an explicit concern with more context.",
                "example": f'remember("concern", "Avoid: {memory_content[:50]}...", rationale="{outcome}")'
            }
            logger.info(f"Memory {memory_id} marked as failed - will be boosted as concern")
