...
MIGRATIONS: List[Tuple[int, str, List[str]]] = [
    ...
    (16, "Add categories JSON column for multi-category support", [
        "ALTER TABLE memories ADD COLUMN categories TEXT DEFAULT '[]';",
        # Backfill: convert single category string to JSON array
        "UPDATE memories SET categories = json_array(category) WHERE category IS NOT NULL AND categories = '[]';",
        # Remap old coding categories to conversational equivalents
        "UPDATE memories SET categories = json_replace(categories, '$[0]', 'fact') WHERE category = 'decision';",
        "UPDATE memories SET categories = json_replace(categories, '$[0]', 'routine') WHERE category = 'pattern';",
