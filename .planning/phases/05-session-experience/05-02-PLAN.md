...
must_haves:
  ...
  key_links:
    ...
    - from: "daem0nmcp/tools/daem0n_briefing.py (_compute_thread_duration)"
      ...
      via: "called from _build_user_briefing on top 2 threads after _get_unresolved_threads returns"
...
**Duration enrichment happens in `_build_user_briefing()`**, NOT inside `_get_unresolved_threads()`. The flow is:
1. `_build_user_briefing()` calls `threads = await _get_unresolved_threads(ctx, user_name)` to get priority-sorted threads
2. After receiving threads, `_build_user_briefing()` runs `asyncio.gather()` on `_compute_thread_duration(ctx, user_name, t["summary"])` for `threads[0:2]` only (top 2 threads, to avoid latency)
3. `_build_user_briefing()` adds `"recurring_since": result` to each of `threads[0:2]` using the gather results
4. This separation keeps `_get_unresolved_threads()` as a pure scoring/filtering function and `_compute_thread_duration()` as a standalone enrichment function
...
**Call `_build_thread_surfacing_guidance()`** in `_build_user_briefing()` after the greeting_guidance is built. Add the result as `response["thread_surfacing_guidance"]` (only if not None).
  ...
  <verify>
Run ALL of the following verification commands:

1. Verify all new functions are defined and importable:
`python -c "from daem0nmcp.tools.daem0n_briefing import _get_unresolved_threads, _compute_thread_duration, _build_thread_surfacing_guidance, _get_follow_up_type; print('All 4 functions importable')"`

2. Verify follow-up type logic:
`python -c "from daem0nmcp.tools.daem0n_briefing import _get_follow_up_type; assert _get_follow_up_type('concern', 2) == 'check_in'; assert _get_follow_up_type('goal', 10) == 'reconnect'; assert _get_follow_up_type('event', 1) == 'outcome'; print('Follow-up types correct')"`

3. Verify inline thread detection was replaced (no raw SQL query for unresolved threads inside _build_user_briefing):
`python -c "import inspect; from daem0nmcp.tools.daem0n_briefing import _build_user_briefing; src = inspect.getsource(_build_user_briefing); assert '_get_unresolved_threads' in src, 'Must call _get_unresolved_threads'; print('Inline detection replaced')"`

4. Verify _get_unresolved_threads is async:
`python -c "import asyncio; from daem0nmcp.tools.daem0n_briefing import _get_unresolved_threads; assert asyncio.iscoroutinefunction(_get_unresolved_threads); print('_get_unresolved_threads is async')"`

All 4 commands must succeed.
  </verify>
  <done>
`_get_unresolved_threads()` returns priority-scored threads with follow_up_type and 90-day cutoff. `_build_user_briefing()` enriches the top 2 threads with `recurring_since` duration via `asyncio.gather()` on `_compute_thread_duration()`. `_build_thread_surfacing_guidance()` generates mid-conversation follow-up guidance. Inline thread detection replaced with call to `_get_unresolved_threads()`.
  </done>
</task>
