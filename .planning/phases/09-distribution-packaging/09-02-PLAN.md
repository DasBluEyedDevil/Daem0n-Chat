...
must_haves:
  truths:
    - "Users can install DaemonChat via a .mcpb bundle dragged into Claude Desktop's Extensions UI"
    - "The .mcpb bundle declares all 8 daem0n_* tools so Claude Desktop knows what capabilities are available"
    - "A build script can reproducibly produce the .mcpb bundle from the project source"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    2. `installer/build_mcpb.py`: Build script that prepares and packages the .mcpb bundle.
       ...
       - NOTE on model downloads: The MCPB/uv path relies on sentence-transformers auto-downloading the model on first use. This is acceptable since MCPB is experimental and uv manages the environment. No explicit model_downloader integration needed here.
