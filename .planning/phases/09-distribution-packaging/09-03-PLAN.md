...
<task type="auto">
  ...
  <action>
    ...
    **[Setup] section:**
    ...
    - SetupIconFile= (comment: set when icon.png is provided)
    ...
    **[Run] section (post-install):**
    ...
    - Parameters: "-m installer.post_install install --python-path ""{app}\python\python.exe"" --install-dir ""{app}"""
    - WorkingDir: "{app}"
    - Flags: runhidden waituntilterminated
    - StatusMsg: "Configuring Claude Desktop..."
    - Set PYTHONPATH environment for the run: use [Run] section with environment manipulation or pass PYTHONPATH via the command. Best approach: set Parameters to include `-c "import sys; sys.path.insert(0, r'{app}\site-packages'); sys.path.insert(0, r'{app}'); exec(open(r'{app}\installer\post_install.py').read())"` -- actually this is complex. Simpler: create a small batch wrapper OR use the Parameters directly: `"""{app}\installer\post_install.py"" install --python-path ""{app}\python\python.exe"" --install-dir ""{app}"""`
    - Actually, simplest correct approach: Filename: "{app}\python\python.exe"; Parameters: """{app}\installer\post_install.py"" install --python-path ""{app}\python\python.exe"" --install-dir ""{app}"""; with environment setup via [Run] section's BeforeInstall or by setting PYTHONPATH in Parameters.

    Use this pattern (proven from research):
    ```
    [Run]
    Filename: "{app}\python\python.exe"; \
    ...
    **[Code] section (Pascal script):**
    - Add a `InitializeSetup` function that checks if Claude Desktop is installed by looking for the Claude AppData directory (`ExpandConstant('{userappdata}\Claude')`). If not found, show a message: "Claude Desktop does not appear to be installed. DaemonChat requires Claude Desktop to function. Install Claude Desktop first from https://claude.ai/download, then run this installer again." Return False to abort.
...
<task type="auto">
  ...
  <action>
    ...
    - `prepare_staging() -> Path`: Orchestrates: create staging dir, download Python, generate requirements, install deps, copy app code. Returns staging dir path.
    ...
    - `if __name__ == "__main__"` with argparse: `--stage-only` (prepare staging without compiling), `--iscc-path` (path to ISCC.exe), `--clean` (remove staging directory first).
  ...
  <done>build_inno.py can prepare a staging directory with Python runtime, dependencies, and app code. It can invoke ISCC.exe if available. All build_inno tests pass.</done>
</task>
...
- build_inno.py can prepare staging directory with all needed files
